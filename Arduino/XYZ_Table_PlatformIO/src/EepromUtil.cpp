...
void EepromUtil::EepromUtil::eeprom_erase_all() {
	char b = 0xff;
	int i;
...
boolean EepromUtil::eeprom_write_string(int addr, const char* string) {
...
boolean EepromUtil::eeprom_read_string(int addr, char* buffer, int bufSize) {
	...
	while ((ch != 0x00) && (bytesRead < bufSize)
			&& ((addr + bytesRead) <= EEPROM_MAX_ADDR)) {
...
boolean EepromUtil::eeprom_write_string(int Addr, String input) {
...
String EepromUtil::eeprom_read_string(int Addr, int length) {
	char cbuff[length + 1];
